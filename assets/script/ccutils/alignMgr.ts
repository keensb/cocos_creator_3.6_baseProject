...
export class alignMgr {
     ...
     * /
     /*
        ...
        topClosesToTop(targetNode, newNode);//顶部对齐
        leftClosesToLeft(targetNode, newNode);//左侧对齐
    ...
    public static alignToFrameSize(node: Node, alignTypeNum: alignType): void {
        ...
        if (alignTypeNum & alignType.LEFT_TO_LEFT) {
            alignMgr.leftClosesToLeft(node, newNode, true);
        }
        else if (alignTypeNum & alignType.LEFT_TO_RIGHT) {
            alignMgr.leftClosesToRight(node, newNode, true);
        }
        else if (alignTypeNum & alignType.RIGHT_TO_RIGHT) {
            alignMgr.rightClosesToRight(node, newNode, true);
        }
        else if (alignTypeNum & alignType.RIGHT_TO_LEFT) {
            alignMgr.rightClosesToLeft(node, newNode, true);
        }

        //竖直方向的对齐同时只有1种能生效
        if (alignTypeNum & alignType.TOP_TO_TOP) {
            alignMgr.topClosesToTop(node, newNode, true);
        }
        else if (alignTypeNum & alignType.TOP_TO_BOTTOM) {
            alignMgr.topClosesToBottom(node, newNode, true);
        }
        else if (alignTypeNum & alignType.BOTTOM_TO_BOTTOM) {
            alignMgr.bottomClosesToBottom(node, newNode, true);
        }
